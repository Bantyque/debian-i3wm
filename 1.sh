...
# ─────────────────────────────────────────────
echo "► Копирование конфигурационных файлов ДО установки софта..."
# ─────────────────────────────────────────────
# Это гарантирует, что ваши пути в .bashrc будут на месте для будущих терминалов
mkdir -p ~/.config ~/.local ~/.moc
[ -d "$REPO_DIR/.config" ] && cp -r "$REPO_DIR/.config/." ~/.config/
[ -d "$REPO_DIR/.moc" ] && cp -r "$REPO_DIR/.moc/." ~/.moc/
[ -d "$REPO_DIR/.local" ] && cp -r "$REPO_DIR/.local/." ~/.local/
[ -f "$REPO_DIR/.bashrc" ] && cp "$REPO_DIR/.bashrc" ~/
[ -f "$REPO_DIR/.Xresources" ] && cp "$REPO_DIR/.Xresources" ~/

# Экспортируем правильные пути для ТЕКУЩЕЙ сессии установщика
...
# ─────────────────────────────────────────────
sudo apt install -y autoconf gcc make pkg-config bc libpam0g-dev libcairo2-dev libfontconfig1-dev libxcb-composite0-dev libev-dev libx11-xcb-dev libxcb-xkb-dev libxcb-xinerama0-dev libxcb-randr0-dev libxcb-image0-dev libxcb-util0-dev libxcb-xrm-dev libxkbcommon-dev libxkbcommon-x11-dev libjpeg-dev
...
# ─────────────────────────────────────────────
echo "► Установка pywal и wpgtk через pipx (Исправлено)..."
# ─────────────────────────────────────────────
# Ставим локально для юзера, без хаков с /usr/local/bin
pipx install pywal
pipx install wpgtk
pipx ensurepath

# Инициализация шаблонов wpgtk напрямую через python-окружение pipx
if [ ! -d "$HOME/.config/wpg" ]; then
    echo "► Инициализация шаблонов wpgtk..."
    ~/.local/share/pipx/venvs/wpgtk/bin/wpg-install.sh -g -i -r -p
fi
...
# ─────────────────────────────────────────────
echo "► Установка betterlockscreen (Исправлено выключение)..."
# ─────────────────────────────────────────────
...
# Создаем СИСТЕМНЫЙ юнит (в /etc/), чтобы он корректно перехватывал sleep.target и не вешал систему
sudo tee /etc/systemd/system/betterlockscreen@.service > /dev/null << EOF
[Unit]
Description=Betterlockscreen dynamic locker
Before=sleep.target
Background=yes
...
# ─────────────────────────────────────────────
echo "► Настройка и включение DM (emptty)..."
# ─────────────────────────────────────────────
sudo apt install -y emptty
# Включаем родной системный сервис из пакета Debian
...
# Выставляем права на исполнение скриптов в скопированном .config
